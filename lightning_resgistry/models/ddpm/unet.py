...
@MODELS.register_module("unet-3D")
class UNet(nn.Module):
    def __init__(self,
        ...
        for i, mult in reversed(list(enumerate(ch_mult))):
            ...
            for _ in range(num_res_blocks + 1):
                ...
                now_ch = out_ch
            if i != 0:
                self.upblocks.append(UpSample(now_ch, up_conv=up_sample_conv, kernel_size=up_sample_kernel_size))
        assert len(chs) == 0
    ...
    def forward(self, x, t):
        ...
        middle_h = h.clone()
        middle_h = middle_h.mean(dim=[2, 3, 4])
        middle_h = self.classier_head(middle_h)
        # Upsampling
